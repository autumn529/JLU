...
def main():
    ...
    for i in tqdm(range(len(test_df))):
        ...
        pred, reason = discriminative_rag_predict(
            row["title"], row["text"],
            indices, category_docs
        )
    ...
    print(f"🎯 Discriminative RAG（无 CoT）准确率: {acc:.2%}")

    final_df.to_excel(
        "Discriminative_RAG_NoCoT_Report.xlsx",
        index=False
    )
